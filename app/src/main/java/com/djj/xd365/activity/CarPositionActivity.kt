...
class CarPositionActivity : AppCompatActivity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        search_car.setOnClickListener {
            startActivityForResult(Intent(this@CarPositionActivity, CarCodeActivity::class.java), 1)
        }
        XD365http.getCarCode(this, xD365Application.constant.driver!!.carid, FutureCallback { e, result ->
            if (e == null && result != null) {
                val mapCar = MapCar(result.get("carcode").asString, null, null, true)
                carlist.add(mapCar)
                startCarPosition()
            }
        })
    ...
    private fun isCharge() {
        ...
        if (resultCar != null) {
            XD365http.getCarPosition(this@CarPositionActivity, resultCar.carcode, FutureCallback { e, result ->
                paycar = resultCar
                initDialog()
//                if (e == null && result != null) {
//                    try {
...
//                        }
//
//                        MaterialDialog.Builder(this@CarPositionActivity)
//                                .title("提示")
...
//                    } catch (e: Exception) {
//                        carlist.remove(resultCar)
//                        Toast.makeText(this@CarPositionActivity, "查询车辆位置失败.", Toast.LENGTH_SHORT).show()
//                        isCharge()
//                    }
//
//                } else {
...
//                }
    ...
    private fun payMoney(mode: JPay.PayMode) {
        val listener = object : JPay.JPayListener {
            ...
            override fun onPaySuccess() {
                Toast.makeText(this@CarPositionActivity, "支付成功", Toast.LENGTH_SHORT).show()
                if (paycar != null) {
                    paycar!!.charge = true
                    isCharge()
                }
            ...
            override fun onPayError(error_code: Int, message: String) {
                Toast.makeText(this@CarPositionActivity, "支付失败>" + error_code + " " + message, Toast.LENGTH_SHORT).show();
                if (paycar != null) {
            ...
            override fun onPayCancel() {
                Toast.makeText(this@CarPositionActivity, "取消了支付", Toast.LENGTH_SHORT).show()
                if (paycar != null) {
                    carlist.remove(paycar!!)
                    isCharge()
                }
        ...
        when (mode) {
            JPay.PayMode.ALIPAY -> {
                ...
                XD365http.getAliPayOrder_CarPosition(this@CarPositionActivity, FutureCallback { e, result ->
                    if (e == null && result != null) {
                        val code = result.get("code").asInt
                        if (code == 0) {
                            val orderInfo = result.get("data").asString
                            JPay.getIntance(this@CarPositionActivity).toPay(JPay.PayMode.ALIPAY, orderInfo, listener)
                        } else {
                            Toast.makeText(this@CarPositionActivity, "生成订单错误", Toast.LENGTH_SHORT).show()
                            if (paycar != null) {
                                carlist.remove(paycar!!)
                                isCharge()
                            }
            ...
            JPay.PayMode.WXPAY -> {
                XD365http.getWxPayOrder_CarPosition(this@CarPositionActivity, FutureCallback { e, result ->
                    if (e == null && result != null) {
                        val code = result.get("code").asInt
                        if (code == 0) {
                            val payParameters = result.get("data").asString
                            //val payParameters = Gson().toJson(result)
                            JPay.getIntance(this@CarPositionActivity).toPay(JPay.PayMode.WXPAY, payParameters, listener)
                        } else {
                            Toast.makeText(this@CarPositionActivity, "生成订单错误", Toast.LENGTH_SHORT).show()
                            if (paycar != null) {
                                carlist.remove(paycar!!)
    ...
    inner class CarPositionTask(private val mapCar: MapCar) {
        ...
        fun run() {
            XD365http.getCarPosition(this@CarPositionActivity, mapCar.carcode, FutureCallback { e, result ->
                if (e == null && result != null) {
                    try {
                        var latLng: LatLng
                        if (result.contains("未知异常，请联系管理员")) {
